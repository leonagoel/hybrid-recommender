...
class TestComputeProductSentiment:
    ...
    def test_with_valid_reviews(self):
        """Test with valid review strings."""
        reviews = ["This is amazing! I love it!", "This is terrible! I hate it!"]
        score = compute_product_sentiment(reviews)
        assert score is not None
        assert isinstance(score, float)
    ...
    def test_with_invalid_reviews(self):
        """Test with reviews containing only non-strings or whitespaces."""
        reviews = ["", "   ", None, 123]  # type: ignore
        assert compute_product_sentiment(reviews) is None
