...
def test_dataset_manager_merge_aggregates_catalog(
        multi_catalog_dataset_manager):
    ...
    assert grouped[grouped['title'] ==
                   'Harry Potter']['catalog'].iloc[0] == 'Books'
    assert grouped[grouped['title'] ==
                   'Star Wars']['catalog'].iloc[0] == 'Movies'
...
def test_content_recommender_catalog_filtering(multi_catalog_dataset_manager):
    ...
    # Recommends cross-domain items (should find Star Wars / The Matrix /
    # Avatar)
    has_movies = any(t in ['Star Wars', 'The Matrix', 'Avatar']
                     for t in all_titles)
    assert has_movies
    ...
    movies_only = recommender.recommend(
        'Dune', top_n=5, target_catalog='Movies')
    for r in movies_only:
        assert r['title'] in ['Star Wars', 'The Matrix', 'Avatar']


def test_collaborative_recommender_catalog_filtering(
        multi_catalog_dataset_manager):
    ...
    books_only = recommender.predict_for_user(
        'user_1', top_n=5, target_catalog='Books')
    for r in books_only:
        # User 1 already interacted with Harry Potter and Dune. The Hobbit is
        # the remaining book.
        assert r['title'] == 'The Hobbit'

    # Personalised recommendations filtered to Movies catalog
    movies_only = recommender.predict_for_user(
        'user_1', top_n=5, target_catalog='Movies')
    for r in movies_only:
        assert r['title'] in ['The Matrix', 'Avatar']


def test_hybrid_recommender_cross_domain_fallback(
        multi_catalog_dataset_manager):
    ...
    # Recommend for a new/unseen title (triggers cold start) filtered by Books
    # catalog
