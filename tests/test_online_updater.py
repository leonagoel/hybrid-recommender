...
def test_online_updater_applies_incremental_updates():
    item_df = pd.DataFrame([
        {'title': 'A', 'rating': 4.0, 'review_count': 2,
            'avg_sentiment': 0.5, 'category': 'X'}
    ])
...
def test_apply_interaction_without_updater_is_ok():
    item_df = pd.DataFrame([
        {'title': 'B', 'rating': 3.0, 'review_count': 0,
            'avg_sentiment': 0.0, 'category': 'Y'}
    ])
    content_df = pd.DataFrame(
        [{'title': 'B', 'description': '', 'top_reviews': [], 'category': 'Y'}])
    hr = HybridRecommender(
        DummyContent(content_df),
        collab_model=None,
        item_df=item_df)
