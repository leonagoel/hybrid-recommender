...
def setup_module():
    ...
    data = [
        {'id': 1,
         'title': 'Red Bicycle',
         'description': 'A red bicycle for kids',
         'category': 'Toys',
         'rating': 4.5,
         'review_count': 10},
        {'id': 2,
         'title': 'Mountain Bike',
         'description': 'An all-terrain mountain bike',
         'category': 'Sports',
         'rating': 4.7,
         'review_count': 50},
        {'id': 3,
         'title': 'Blue Bicycle',
         'description': 'A blue bicycle with basket',
         'category': 'Toys',
         'rating': 4.2,
         'review_count': 5},
    ]
    df = pd.DataFrame(data)
    df['combined'] = df['title'].astype(
        str) + ' ' + df['description'].astype(str) + ' ' + df['category'].astype(str)
