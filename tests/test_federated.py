...
def test_federated_client_factor_calculation():
    ...
    client_empty = FederatedClient(
        user_id="user2", private_ratings={
            "Item Unknown": 5.0})
...
def test_federated_client_item_updates():
    ...
    updates = client.compute_local_item_updates(
        global_item_factors, title_to_idx, reg=0.05)

    # Updates should contain 'Item A' but NOT 'Item B' because the user only
    # rated 'Item A'
...
def test_federated_server_aggregation():
    ...
    client2_updates = {"Item A": np.array(
        [0.3, 0.4, 0.5]), "Item B": np.array([0.2, 0.4, 0.6])}
...
def test_train_federated_collaborative_model():
    ...
    assert recommender.item_factors.shape == (
        2, 3)  # 2 factors, 3 unique items
    assert recommender.user_factors.shape == (
        3, 2)  # 3 unique users, 2 factors
