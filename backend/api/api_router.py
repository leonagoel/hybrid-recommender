from fastapi import APIRouter, Depends, Query, Request, Response, HTTPException, WebSocket, WebSocketDisconnect, UploadFile, File
...
from backend.db import get_supabase, get_supabase_admin
from backend.core.config import MAX_SEARCH_QUERY_LENGTH, CACHE_MAX_ENTRIES, CACHE_TTL_SECONDS
from backend.core.state import models, MODEL_REGISTRY, ACTIVE_MODEL_VERSION, SHADOW_MODEL_VERSION, STAGING_MODEL_VERSION, SHADOW_LOGS, generate_model_version, realtime_hub
from backend.core.cache import _get_cached_response, _set_cached_response, _clear_response_cache, _cache_key
from backend.core.rate_limit import _apply_rate_limit
from backend.core.security import _require_admin_access, _admin_access_dep
from backend.schemas.schemas import WeightsUpdate, PurchaseCreate, FeedbackCreate, RealtimeRecommendationRequest, FederatedTrainRequest
from backend.services.ml_service import cold_start_recommendation, _precompute_recommendation_cache
from backend.csrf import CSRFTokenResponse, generate_csrf_token, set_csrf_cookie
from backend.core.metrics import get_response_metrics_snapshot
...
import hmac
import hashlib
from backend.core.config import MOCK_PRODUCTS
...
@router.post("/api/train/federated")
def train_federated(
...
@router.get("/api/recommend/{item_title}")
...
@router.get("/api/recommend/cold_start")
...
@router.post("/api/realtime/behavior")
...
# ── Similarity Matrix ─────────────────────────────────────────────────
@router.get("/api/similarity-matrix")
...
@router.post("/api/models/{version}/promote")
def promote_model(
    version: str,
    _csrf: None = Depends(csrf_header_dep),
    _admin: None = Depends(_admin_access_dep),
...
@router.post("/api/models/{version}/shadow")
...
# ── Purchases ─────────────────────────────────────────────────────────
@router.get("/api/purchases/{user_id}")
