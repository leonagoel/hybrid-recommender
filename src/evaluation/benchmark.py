...
class PopularityRecommender:
    def __init__(self, item_df):
        if 'rating' in item_df.columns:
            self.popular_items = item_df.sort_values(
                'rating', ascending=False)['title'].tolist()
        else:
...
def run_benchmark():
    ...
    models = {
        ...
        # Causal variant — same weights as Semantic-Hybrid but with IPS
        # debiasing
    ...
    for name, model in models.items():
        ...
        for user_id, query_item, relevant_items in test_pairs:
            if hasattr(
                    model, 'predict_for_user') and name == "SVD-Collaborative":
                ...
                # the causal one applies IPS internally before returning
                # results
                recs_raw = model.recommend(
                    query_item, user_id=user_id, top_n=K)
            else:
            ...
            try:
                ild = _intra_list_diversity(
                    rec_titles, item_df, content_model.matrix)
            except Exception:
