...
class HybridRecommender:
    ...
    def select_bandit_arm(self):
        ...
        if random.random() < getattr(self, 'epsilon', 0.1):
            return random.randint(0, len(getattr(self, 'bandit_arms', [0])) - 1)

        best_arm = max(
            self.arm_rewards,
            key=lambda x: getattr(self, 'arm_rewards', {})[x] / max(getattr(self, 'arm_counts', {})[x], 1)
        )
        return best_arm
    ...
    def update_bandit_reward(self, arm_id, reward):
        ...
        return {'alpha': self.alpha, 'beta': self.beta, 'gamma': self.gamma, 'delta': getattr(self, 'delta', 0)}
