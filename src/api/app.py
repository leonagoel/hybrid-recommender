...
for key in ("content_model", "collab_model", "hybrid_model",
            "adapted_df", "meta", "uploaded_file_name", "explainer"):
...
with st.sidebar:
    ...
    if total_weight <= 0:
        st.warning(
            "All weights are set to zero. Please increase at least one weight to see the normalized distribution.")
    else:
        normalized_weights = {
    ...
    if total_weight == 0:
        ...
        st.warning(
            "All raw weights are zero, so the model will use equal weights.")
    else:
        effective_weights = {
...
if uploaded_file is not None:
    ...
    if st.session_state.adapted_df is not None:
        ...
        with st.expander("Detected columns"):
            detected = {k: v for k, v in meta.items(
    ...
    if st.button("🔨 Build Models"):
        ...
        with st.spinner("Building models — this may take a moment for large datasets…"):
            try:
                ...
                # Hybrid model — pass causal_config for structured
                # configuration
                ...
                if collab_model is not None:
                    st.success(
                        "✅ Content model and Collaborative model trained. Hybrid mode active.")
                else:
                    st.success(
                        "✅ Content model trained. Collaborative model skipped (dataset needs more than one unique user).")
        ...
        if not query.strip():
            ...
            try:
                if is_user_id and collab_model is not None:
                    ...
                    recs_raw = collab_model.predict_for_user(
                        query, top_n=top_n)
                    ...
                    exact = title_series[title_series.str.lower()
                                         == query.lower()]
                    ...
                    if exact.empty:
                        ...
                        st.info(
                            f"Exact match not found. Using closest match: **{item_title}**")
                    else:
                        item_title = exact.iloc[0]

                recs = hybrid_model.recommend(
                    item_title, top_n=top_n, explain=True)
                ...
                if not recs:
                    ...
                    for i, rec in enumerate(recs, start=1):
                        ...
                        # Show causal score column only when debiasing was
                        # active
                        causal_active = (
                        ...
                        if causal_active:
                            ...
                            # user can see how much the causal layer changed
                            # the rank
                            original = rec.get("original_score")
                            causal = rec.get(
                                "causal_score", rec.get("hybrid_score"))
                            delta = round(
                                causal - original, 4) if original is not None else None
                            col_causal.metric(
                                "🔬 Causal",
                                causal,
                                delta=delta,
